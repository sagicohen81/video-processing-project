...
""" This function gets image: the gradient of the PDF and scribble points,
    and treats the scribble points as "green points": points that we calculate the distance from,
    and calculates cost for every other pixel in the gradient image.
    This is the only function we modified in this module.
"""
def map_image_to_costs(image, scribble_pts):
    ...
    #exits = np.where(data[:] > 0.4 and data[:] < 0.5)
    exits = np.array(scribble_pts)
    ...
    space = np.ones(image.shape, dtype=np.bool)
    #space[obstacles] = False
    space[(exits[:,1],exits[:,0])] = False
    # Cost field: Inversely proportional to greyscale values
    cost_field = np.empty(data.shape)
    #cost_field[obstacles] = np.inf
    #cost_field[space] = np.exp(cost_field_exp_coef * image[space]) + np.random.random(size=image[space].shape)
    cost_field[space] = np.exp(cost_field_exp_coef * image[space]) + np.random.random(size=image[space].shape)
    #cost_field[space] = np.where(cost_field[space] > 50, 50, cost_field[space])
    cost_field[(exits[:,1],exits[:,0])] = 0
    #cost_field[space] = 1. / (255 * grey_scales[space])

    #cost_field[space] = image[space]
    return cost_field
...
def _propagate_distance(cell, costs, wdt_field):
    ...
    x_high = (2 * c) / (-b - math.sqrt(max(D, 10**-10)))
    return x_high